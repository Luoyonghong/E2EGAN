...
class gru(object):
    model_name = "GRU_Phy"
    def __init__(self, sess, args, train_x, test_x, train_y, test_y):
        self.lr = args.lr            
        self.sess=sess
    ...
    def RNN(self,X, X_lengths,Keep_prob, reuse=False):
        #       2*3*2
        ...
        # if lastvalues is zero, take mean as it
         ...
         with tf.variable_scope("gru", reuse=reuse):
            ...
            grucell = GRUCell(self.n_hidden_units)
            init_state = grucell.zero_state(self.batch_size, dtype=tf.float32) # 初始化全零 state
            outputs, final_state = tf.nn.dynamic_rnn(grucell, X, \
    ...
    def build(self):
        ...
        self.pred = self.RNN(self.x, self.x_lengths, self.keep_prob)
    ...
    def nextBatch(self,dataset_x, dataset_y):
        i=1
        while i*self.batch_size <= len(dataset_x):
            x=[]
            y=[]
            for j in range((i-1)*self.batch_size,i*self.batch_size):
                x.append(dataset_x[j])
                y.append(dataset_y[j])
            i+=1
            yield  x,y

    def model_dir(self,epoch):
        return "{}_{}_{}_{}/epoch{}".format(
    ...
    def train(self):
        ...
        if os.path.exists(os.path.join(self.checkpoint_dir, self.model_dir(self.epoch), self.model_name)):
            for nowepoch in range(1,self.epoch+1):
                ...
                acc,auc,model_name=self.test(nowepoch)
                if auc > max_auc :
                    max_auc = auc 
        ...
        while epochcount<self.epoch:
            self.shuffle() 
            for data_x,data_y in self.nextBatch(self.x_train,self.y_train):
                data_x_lengths = [48]*self.batch_size 
            ...
            acc,auc,model_name=self.test(epochcount)
            if auc > max_auc :
                max_auc = auc 
    ...
    def test(self,epoch):
        ...
        totalauc=0.0
        auccounter=0
        for data_x,data_y in self.nextBatch(self.x_test,self.y_test):
            data_x_lengths = [48]*self.batch_size 
